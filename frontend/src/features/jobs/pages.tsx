...
function isJob(value: unknown): value is Job {
  return typeof value === 'object' && value !== null && 'questions' in value
}

function isInvitation(value: unknown): value is Invitation {
  return typeof value === 'object' && value !== null && 'participantEmail' in value
}

function isScheduledInterview(value: unknown): value is ScheduledInterview {
  return typeof value === 'object' && value !== null && 'startsAt' in value && 'attendees' in value
}
...
export function JobsPage() {
  const { jobs, loading, error, reload, participants } = useData()
  ...
  return (
    <>
      ...
      <section className="metrics">
        ...
        <Metric label="Participants" value={String(participants.length)} />
        <Metric
          label="Assessments"
...
export function JobDetailPage() {
  ...
  useDomainEvent<Job | Invitation | ScheduledInterview | undefined>(`jobs/${id}`, (event) => {
    ...
    if (isInvitation(data)) {
      setInvitations((current) => upsertById(current, data))
      return
    }
    if (isScheduledInterview(data)) {
      setScheduledInterviews((current) => upsertById(current, data))
      return
    }
    if (event.type === 'job.question-attached' || event.type === 'job.question-detached') {
      void api.jobs
        .get(id)
        .then(setJob)
  ...
  useDomainEvent<Invitation>(`job-invitations/${id}`, (event) => {
    setInvitations((current) => upsertById(current, event.data))
  })
  ...
  return (
    <>
      ...
      {tab === 'questions' && <JobQuestionsView job={job} />}
...
function JobSettings({ job }: { job: Job }) {
  ...
  async function save(event: SyntheticEvent<HTMLFormElement>) {
    ...
    setError('')
    try {
      await api.jobs.update(job.id, { status, durationMinutes })
    } catch (requestError) {
      setError(errorMessage(requestError))
    } finally {
      setSaving(false)
...
function JobQuestionsView({ job }: { job: Job }) {
  const [query, setQuery] = useState('')
...
function InvitationsView({
  ...
  async function send(participantId: number) {
    ...
    setError('')
    try {
      await api.jobs.invite(job.id, participantId)
    } catch (requestError) {
      setError(errorMessage(requestError))
    } finally {
...
function InvitationAnswerReview({
  ...
  useDomainEvent<Invitation>(`assessment-invitations/${interview.invitation.id}`, (event) => {
    setCurrent((value) => ({ ...value, invitation: event.data }))
  })
  ...
  return (
    <section className="answer-review" aria-labelledby="answer-review-title">
      <div className="section-heading">
        ...
        {current.invitation.status === 'completed' && (
          <div className="outcome-actions" aria-label="Candidate outcome">
            ...
            {current.invitation.outcome && <StatusBadge value={current.invitation.outcome} />}
          </div>
        )}
        {error && <ErrorAlert message={error} />}
        <div className="review-heading-actions">
          <StatusBadge value={current.invitation.status} />
      ...
      <div className="review-question-list">
        {current.job.questions.map((question, index) => {
          const answer = current.answers[String(question.id)]?.trim()
          return (
